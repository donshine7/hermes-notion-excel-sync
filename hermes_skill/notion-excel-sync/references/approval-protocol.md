...
- After Notion confirms the approved correction, append it to the Wiki approved
  overlay.
- Bind the overlay to the cited source-cell basis. Preserve it across unrelated
  workbook edits and flag a conflict when the cited basis changes.

Independent corrections use only this strict JSON request shape:

```text
/nx_correct {"database":"...","entity_key":"...","property":"...","value":...,"reason":"...","case_number":"..."}
```

`case_number` is optional; the other keys are required. This request is never approval.
Do not infer approval from prose; use the separate exact `/nx_approve` command generated
